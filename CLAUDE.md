...
- **Data:** Redis (HOSTUP-internal via `ioredis`, with legacy Upstash REST fallback where explicitly configured) is the source of truth for worker-owned payloads via [src/lib/data-store.ts](src/lib/data-store.ts) - three-tier read (Redis -> bundled file -> in-memory last-known-good). Set `REDIS_URL` or `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` - never both. Production source freshness is owned by the HOSTUP worker fleet, not GitHub duplicate scrapers.
...
- Copy `.env.example` to `.env.local`. Required for prod: `GITHUB_TOKEN`, `CRON_SECRET`. Pick exactly ONE Redis pair: `REDIS_URL` (HOSTUP Redis) OR `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` (Upstash) — never both. Without either, the data-store gracefully falls back to bundled JSON + memory.
...
## Where to Look First
- **UI / design work (read FIRST before touching any visual surface)** → [docs/DESIGN-SYSTEM.md](docs/DESIGN-SYSTEM.md) — bulletproof contract for tokens, typography, body atmosphere, component primitives, icons, brand, charts, freshness chrome, debt log, change rules, handoff protocol. Adopted from "Trendingrepo Revive" package 2026-05-23. Source-of-truth file: `public/shell.css`. Forbidden: inline hex in TSX/CSS, `@import` Google Fonts (use `next/font`), creating new CSS files outside shell.css.
...
- **2026-06-01 production hardening handoff** → [docs/HANDOVER-2026-06-01-PRODUCTION-HARDENING.md](docs/HANDOVER-2026-06-01-PRODUCTION-HARDENING.md) — current HOSTUP image tags, zero-tolerance health proof, worker/source root causes, Reddit-off decision, and remaining token-rotation action.
...
- **SEO / GEO work (answer-surfaces, llms.txt, schema, sitemaps)** → [docs/GEO-ANSWER-SURFACES.md](docs/GEO-ANSWER-SURFACES.md) — the playbook + reusable JSON-LD primitives (`src/lib/seo/structured-data.ts`, `src/components/seo/*`) + the 200-only citation contract + `scripts/geo-citation-probe.mjs`. Wave 1+2 (`/categories`, `/best`, `/compare`, `/alternatives`, `/collections`, `/glossary`, `/blog` + repo-page enrichment) is LIVE on prod — deployed during the 2026-06-01 HOSTUP hardening wave; commit refs `e22db2bae` / `89b551f2f` / `1f5efbd52`. Local `geo-answer-surfaces` skill mirrors it. **GSC visibility**: programmatic Search Console access shipped 2026-06-01 — `npm run gsc:audit` runs the baseline + sitemap + URL-inspection sweep and writes `data/_geo/gsc-*.json`; weekly cron at `.github/workflows/cron-gsc-audit.yml` keeps the snapshots fresh. **Pre-deploy SEO probe**: `npm run check:seo` (against prod) or `npm run check:seo:local` (against `localhost:3023`) asserts the metadata invariants whose violation cost ~95% impressions on 2026-05-30/31 (dual `<meta robots>` tags on soft-404s, canonical-to-/, noindex on real pages). Wired into post-deploy smoke. See `tasks/gsc-baseline-2026-06-01.md` for the operator setup (service-account creation + GSC user-add) and `tasks/gsc-incident-2026-06-07.md` for the noindex hotfix post-mortem.
...
- Don't switch Twitter collector back to API mode — it silently fails on ephemeral route filesystems.
...
- Don't `readFileSync(process.cwd(), "data", ...)` for new data sources — use the data-store. The reason filesystem reads worked at all is that bundled JSON is baked into each deploy; that coupled data freshness to deploys and caused 17-34 deploys/day from data churn alone (commit `87e3f4e`, 2026-04-26).
...
- **Collectors must keep last-50, never empty the cache (2026-05-08 rule).** When a scraper writes its output JSON, it must read existing → union with new batch → dedupe → keep top 50. Never write fewer than `min(50, existing.length)`. Why: `/reddit/trending` "0 0" and `/twitter` "only 16" both happened today because scrapers wrote fewer rows than the page filter required. Surfaces should always show last-50 cached and degrade gracefully to slightly-stale rather than empty. See [docs/INGESTION.md](docs/INGESTION.md#rule-keep-last-50-cache-2026-05-08).
- **`git stash -u` creates orphan untracked-files commits.** Visible only via `git fsck --no-reflogs --lost-found`. Today's repo had ~30 of these; the rescue stash `004777a8` was the source of truth for the entire repo-detail redesign + auth/db scaffolding even though it wasn't reachable from any branch. If you need to switch context, prefer `git checkout -b wip/<topic>` + commit instead of `git stash -u`.
- **Next 15 forbids non-handler exports from `route.ts`.** `deriveHealth` / `verifyWebhookSecret` exported from a `route.ts` fail the build at the `.next/types/.../route.ts` guard ("Property 'X' is not assignable to type 'never'"). Allowed exports: `GET/POST/PUT/PATCH/DELETE/HEAD/OPTIONS`, `runtime`, `dynamic`, `revalidate`, `maxDuration`, `dynamicParams`, `fetchCache`, `preferredRegion`, `config`, `generateStaticParams`. Helpers must move to a sibling lib file.
- **Drizzle pages can't statically prerender without `DATABASE_URL`.** The db client throws on first property access. ISR (`revalidate`) doesn't help — it still prerenders at build. Mark Drizzle-using pages `export const dynamic = "force-dynamic"`.
...
- **Worker fetchers must NOT overwrite their cache with an empty payload (2026-05-27 rule).** Same class as the collector keep-last-50 rule but for `apps/trendingrepo-worker/src/fetchers/`. A fetcher that writes `{items: []}` when all upstream calls fail silently zeros the user-facing slug (this is exactly why `recent-repos` showed 0 for weeks). Every worker fetcher must read existing → union → cap via `mergeAndCap` / `shouldPreserveCache` from [apps/trendingrepo-worker/src/lib/util/cache-merge.ts](apps/trendingrepo-worker/src/lib/util/cache-merge.ts), OR be on the documented allow-list in [scripts/check-worker-keep-last-50.mjs](scripts/check-worker-keep-last-50.mjs) (wired into `lint:guards`). 11 fetchers are `AUDIT-PENDING` — migrate the per-source mention collectors (bluesky/hackernews/devto/lobsters) first.
- **Never sort registry repos by `lastSeenAt` alone — add a `fullName` tiebreaker.** The `repo-registry` hourly tick updates ~700 repos to the SAME `lastSeenAt` timestamp, so `Array.sort` on that key is non-deterministic: the same N get picked every run and the alphabetic tail never gets enriched. The shared `rankedRegistryFullNames` helper ([apps/trendingrepo-worker/src/lib/util/registry-candidates.ts](apps/trendingrepo-worker/src/lib/util/registry-candidates.ts)) has the tiebreaker; any local picker (e.g. `repo-community-profile`'s own `pickProfileCandidates`) must too. When fixing duplicated selection logic, grep the whole worker for parallel implementations FIRST.
- **GH classic-PAT expiration is the leading failure indicator for the worker.** `GH_TOKEN_POOL` (on TOOLBOX `/opt/toolbox-trendingrepo-worker/.env`) is a comma-separated pool. On 2026-05-26 all 10 PATs expired the same day → `recent-repos=0`, registry growth stalled, every GH-API fetcher silently 401'd. Mitigation: the pool is now 20 PATs (50× rate-limit headroom). When recent-repos/community-profile/star-activity return empty, FIRST check token health: `ssh toolbox 'grep "^GH_TOKEN_POOL=" /opt/toolbox-trendingrepo-worker/.env | cut -d= -f2- | tr "," "\n" | wc -l'` then test one against `https://api.github.com/user` (expect 200, not 401). Rotation recipe in [docs/WORKER-HARDENING-2026-05-27.md](docs/WORKER-HARDENING-2026-05-27.md).
- **Reddit is intentionally disabled in production.** Do not re-enable Reddit jobs, breakers, or workflow producers unless there is an approved credentialed producer that cannot write empty payloads. Current `health:prod` expects `reddit` in disabled source-health entries.
- **Source-health cold breakers are informational, not degraded.** Process-local breakers reset on app restart; never fake successes from Redis to warm them. Health fails only when breakers are open/half-open or worker Redis freshness is non-green.
...
- Swarm branches do not imply a production deploy. Use local build/start probes and HOSTUP/Cloudflare smoke checks for release proof.
