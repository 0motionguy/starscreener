...
- **Twitter** reads through the TOOLBOX engine (`--provider toolbox` → `POST api.aiso.tools/v1/skills/twitter.nitter_search/run`, needs `TOOLBOX_API_URL` + `TOOLBOX_API_KEY`). Apify is policy-off (docs/POLICY-NO-APIFY.md); cookie-based providers are dead post-2026 anti-bot. Engine rows carry no engagement metrics (nitter RSS source) — mention counting + cross-signal work fully, engagement-ranked surfaces see zeros.
...
- Local collectors: `npm run collect:twitter` (toolbox engine, NOT scrape:twitter), `npm run scrape:reddit` / `:hn` / `:bsky` / `:ph` / `:devto` / `:lobsters` / `:arxiv` / `:npm`
