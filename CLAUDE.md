...
- **SEO / GEO work (answer-surfaces, llms.txt, schema, sitemaps)** → [docs/GEO-ANSWER-SURFACES.md](docs/GEO-ANSWER-SURFACES.md) — the playbook + reusable JSON-LD primitives (`src/lib/seo/structured-data.ts`, `src/components/seo/*`) + the 200-only citation contract + `scripts/geo-citation-probe.mjs`. Wave 1+2 (`/categories`, `/best`, `/compare`, `/alternatives`, `/collections`, `/glossary`, `/blog` + repo-page enrichment) is LIVE on prod — landed on `main` via commits `e22db2bae` / `89b551f2f` / `1f5efbd52` and serves 200s from `trendingrepo.com`. Local `geo-answer-surfaces` skill mirrors it. **GSC visibility**: programmatic Search Console access shipped 2026-06-01 — `npm run gsc:audit` runs the baseline + sitemap + URL-inspection sweep and writes `data/_geo/gsc-*.json`; weekly cron at `.github/workflows/cron-gsc-audit.yml` keeps the snapshots fresh. See `tasks/gsc-baseline-2026-06-01.md` for the operator setup (service-account creation + GSC user-add) and the current indexing-coverage gaps the audit surfaced.
...
- **Vercel ISR caches both 2xx AND 5xx — stuck-500 routes survive deploys.** Cache headers on ISR routes default to `s-maxage=300, stale-while-revalidate=31535700` (≈ 1 year SWR). A transient 500, OR one cached BEFORE a fix shipped, persists for **weeks** even after the underlying bug is on `main` and Vercel has rebuilt — every subsequent request returns the stale cached error from edge cache. Symptom: deploy URL returns 200 for a route, prod alias returns 5xx for the same route. Discovered 2026-05-13 — 5 cookbook `/repo/*` routes stayed 500 on prod after #1159 fixed the cold-miss bug; deploy URL was 200 for all 5. **30-second runbook**: (1) confirm `curl -sI https://<deploy-url>/<route>` returns 200 while `curl -sI https://trendingrepo.com/<route>` returns 5xx → diagnosis confirmed; (2) `curl -X POST https://trendingrepo.com/api/revalidate -H "Authorization: Bearer $CRON_SECRET" -H "Content-Type: application/json" -d '{"paths":["/<route>"]}'` (endpoint shipped in #1213); (3) re-curl prod → expect 200 within seconds. The post-deploy smoke workflow auto-flushes via the same endpoint after #1221, so this manual recipe should mostly only be needed for routes outside the smoke probe list.
