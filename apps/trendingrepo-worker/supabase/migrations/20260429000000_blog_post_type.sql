-- =========================================================================
-- TrendingRepo: AI-blog 'post' item type + per-post boost in trending_score()
-- 2026-04-29 (already applied to agnt-prod via mcp.apply_migration)
-- =========================================================================
...
begin
  if not exists (
    ...
    alter type trending_item_type add value 'post';
  end if;
end $$;
    ...
    update trending_items t
       set trending_score =
             ...
             + case
                 ...
                 when type_row = 'post'::trending_item_type then
                     case when (b.raw->>'lab_id') = any(frontier_labs) then 0.10
                          when (b.raw->>'lab_id') = any(strong_labs)   then 0.05
                          else 0 end
                   + case
                       when jsonb_typeof(b.raw->'arxiv_ids') = 'array'
                            and jsonb_array_length(b.raw->'arxiv_ids') >= 1
                       then 0.10
                       else 0
                     end
                   + case
