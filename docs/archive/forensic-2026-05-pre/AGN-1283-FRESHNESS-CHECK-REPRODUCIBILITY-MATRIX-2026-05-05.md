...
## Blocking source consistency analysis
- Consistency result for this 5-run burst: endpoint-level failure is consistent (`5/5` non-zero with HTTP 500), but failing endpoint is not single-source stable.
- Endpoint split:
  - `/api/health?soft=1`: 3 failures (60%)
  - `/api/cron/freshness/state`: 2 failures (40%)
- Interpretation: gating failure is reproducible at the product layer, while the specific failing freshness path alternates. This points to shared backend freshness-state instability rather than localhost availability drift.
