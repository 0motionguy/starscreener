...
### Minute-level collision windows (fresh expansion)
Expanded from workflow cron minute fields in this heartbeat:
- `:00` -> 18 scheduled jobs (highest burst window)
- `:30` -> 10 scheduled jobs
- `:17` -> 7 scheduled jobs
- `:15` -> 6 scheduled jobs
- `:45` -> 5 scheduled jobs
- `:05`, `:20`, `:25`, `:35`, `:55` -> 4 each
...
## Shared-key writer matrix with risk tags
| Key family | Workflow/script writer | Worker writer | Risk tag |
|---|---|---|---|
| `collection-rankings` | `refresh-collection-rankings.yml` -> `scripts/scrape-trending.mjs --only-collection-rankings` | `fetchers/collection-rankings/index.ts` | `high` |
| `trending`, `trending-lite` | `scrape-trending.yml` -> `scripts/scrape-trending.mjs` | `fetchers/oss-trending/index.ts` | `high` |
| `devto-mentions`, `devto-trending` | `scripts/scrape-devto.mjs` | `fetchers/devto/index.ts` | `high` |
| `reddit-mentions`, `reddit-all-posts` | `scripts/scrape-reddit.mjs` (via scrape-trending lane) | `fetchers/reddit/index.ts` | `high` |
| `producthunt-launches` | `scripts/scrape-producthunt.mjs` | `fetchers/producthunt/index.ts` | `medium` |
| `trustmrr-startups`, `revenue-overlays` | `scripts/sync-trustmrr.mjs` | `fetchers/trustmrr/index.ts` | `high` |
| `deltas` | `scripts/compute-deltas.mjs` (from scrape-trending workflow) | `fetchers/deltas/index.ts` | `high` |

## Top 5 overlap risks + concrete mitigation options
1. `:00` minute burst (18 jobs) raises contention/failure coupling.
   - Mitigation: move non-critical daily jobs off `:00` to staggered `:08/:12/:22/:42` windows.
2. Dual-writer on `trending*` + `deltas` can cause stale/new flips.
   - Mitigation: declare worker as primary writer for these keys and convert GHA lane to manual recovery only.
3. Dual-writer on `collection-rankings` keeps conflict surface active every 6h.
   - Mitigation: keep only one active scheduler path (prefer worker); keep workflow as dispatch-only break-glass.
4. Dual-writer on `trustmrr*` and `revenue-overlays` with near-overlapping windows.
   - Mitigation: single writer owner + explicit fallback runbook with guarded manual dispatch.
5. Freshness monitor duplication (`*/15` multiple workflows) can create duplicate alert noise.
   - Mitigation: retain one blocking monitor and move others to advisory telemetry-only mode.
...
### Explicit unblock criteria (operator action checklist)
1. GitHub auth:
   - Command: `gh auth login` (or set valid `GH_TOKEN`), then run `gh run list --limit 10`.
   - Expected: command returns run rows, not HTTP 401.
2. Local freshness runtime:
   - Command: `npm install` then `npm run freshness:check`.
   - Expected: no `'tsx' is not recognized` error.
3. Local endpoint reachability:
   - Command: `Invoke-WebRequest http://localhost:3023/api/health?soft=1`.
   - Expected: HTTP 200/500 response body from app (not connection failure).
