...
- Timestamp: 2026-05-05T15:32:56+08:00
...
Classification:
- This heartbeat failure is a local runtime reachability failure (localhost:3023 timeout), not a product-freshness verdict.
...
Evidence:
- `GET $PAPERCLIP_API_URL/health` -> `Unable to connect to the remote server`
- `GET $PAPERCLIP_API_URL/api/companies/$PAPERCLIP_COMPANY_ID/agents` -> `Unable to connect to the remote server`
...
- Unblock owner/action: platform-network owner restores reachability to `http://192.168.192.1:3100`.
