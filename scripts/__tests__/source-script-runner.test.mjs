...
test("runAsRegisteredSource: warns on unknown sourceId but still runs", async () => {
  _resetForTests();
  const cap = captureStreams();
  try {
    const result = await runAsRegisteredSource({
    ...
    const warned = cap.err.some((line) =>
      line.includes("[source-script-runner] warn: sourceId"),
    );
    assert.ok(
      warned,
      "expected an unregistered-source warning on stderr",
    );
    const summary = cap.out.find((line) => line.startsWith("[run-summary]"));
