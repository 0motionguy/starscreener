...
const ALLOW = new Map([
  ...
  // Migrated 2026-05-28 (removed from this allow-list — now detected as
  // compliant via readDataStore + shouldPreserveCache): hackernews, bluesky,
  // devto, lobsters, twitter, oss-trending, consensus-trending, repo-metadata,
  // repo-profiles. Each reads its prior slug and preserves it on an empty scan.
  ['collection-rankings', 'Has an explicit totalRows>0 zero-write guard (preserves the slug on a total OSSInsight outage). Per-tick snapshot writer with no read→merge semantics, so it stays allow-listed. Migrated 2026-05-28.'],
