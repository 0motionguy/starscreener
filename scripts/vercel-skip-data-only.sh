#!/usr/bin/env bash
# Vercel ignoreCommand: skip builds that cannot affect the deployed app.
# Exit 0 = skip build. Exit 1 = build.
#
# Vercel sets VERCEL_GIT_COMMIT_REF (branch) + VERCEL_GIT_PREVIOUS_SHA
# (last deployed SHA for the project/branch).
...
# These paths are validated by GitHub CI or are runtime data/docs. They do not
# need a Vercel build/deploy by themselves. Keep this conservative: app source,
# config, package/dependency, public asset, and Vercel config changes still
# build.
skip_only_re='^(\.data/|data/|docs/|\.github/|tests/|perf/|scripts/|\.tmp/|playwright-report/|test-results/|coverage/)|(^|/)(README|CHANGELOG|CLAUDE|AGENTS)\.md$|^(\.gitleaksignore|\.gitignore|eslint\.config\.mjs|playwright\.config\.ts)$'

runtime_changed=$(echo "$changed" | grep -vE "$skip_only_re" || true)

if [ -z "$runtime_changed" ]; then
  echo "[ignoreCommand] non-runtime-only change detected - skipping build"
  echo "$changed" | head -10 | sed 's/^/  - /'
  exit 0
fi

echo "[ignoreCommand] runtime paths changed - building"
echo "$runtime_changed" | head -10 | sed 's/^/  - /'
exit 1
