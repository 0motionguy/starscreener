...
export default function AdminKeysError({ error, reset }: ErrorProps) {
  useEffect(() => {
  ...
  return (
    <div className="max-w-[640px] mx-auto px-4 md:px-6 py-16 md:py-24 text-center">
      ...
      >
        Admin keys page crashed.
      </h1>
      <p
        className="leading-relaxed mb-5"
        style={{ fontSize: 14, color: "var(--v2-ink-300)" }}
      >
        This surface failed to render. Try again, or head back to the admin index if it keeps erroring.
      </p>
      {error.digest && (
        <p
      ...
      <div className="flex flex-wrap items-center justify-center gap-2">
        ...
        <Link href="/admin" className="v2-btn v2-btn-ghost inline-flex">
          <ArrowLeft className="h-4 w-4" style={{ marginRight: 8 }} />
          BACK TO ADMIN
        </Link>
      </div>
    </div>
