...
export async function GET(): Promise<Response> {
  ...
  const header = [
    ...
    "",
    `Live snapshot — last refresh: ${lastFetched}. Source: aggregated GitHub + HN + Bluesky + dev.to + ProductHunt + Lobsters signals.`,
    "",
    "---",
  ].join("\n");
  ...
  const dataFreshness = [
    ...
    "- Underlying active data sources span GitHub, Hacker News, ProductHunt, Bluesky, dev.to, Lobsters, arxiv, npm, OpenRouter, funding feeds, and model/package registries",
  ].join("\n");
  ...
  const definitions = [
    ...
    "- **Cross-signal breakout** — a repo firing on >= 3 distinct active platforms (GitHub, HN, ProductHunt, Bluesky, dev.to, Lobsters) inside the same 24h trending window.",
