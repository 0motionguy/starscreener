...
test("OPTIONS /portal denies disallowed origin without ACAO reflection", () =>
  withPortalEnv(() => {
    const req = new Request("http://localhost:3023/portal", {
      method: "OPTIONS",
      headers: {
...
test("OPTIONS /portal allows configured origin", () =>
  withPortalEnv(() => {
    const req = new Request("http://localhost:3023/portal", {
      method: "OPTIONS",
      headers: {
