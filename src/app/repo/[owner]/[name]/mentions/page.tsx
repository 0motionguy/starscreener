...
function buildPerSourceTimeline(
  mentions: MentionItem[],
  source: MentionSource,
): Array<{ day: string; count: number }> {
  return buildTimeline(mentions.filter((item) => item.source === source));
}
...
export default async function RepoMentionsPage({ params }: PageProps) {
  ...
  const mentions24h = mentions.filter(
    (item) => Date.now() - Date.parse(item.createdAt) <= DAY_MS,
  ).length;
  ...
  const itemListJsonLd = {
    ...
    "@type": "ItemList",
    name: `${profile.repo.fullName} mentions`,
    url: absoluteUrl(`/repo/${profile.repo.owner}/${profile.repo.name}/mentions`),
    itemListOrder: "https://schema.org/ItemListOrderDescending",
    numberOfItems: mentions.length,
    itemListElement: mentions.slice(0, 100).map((item, index) => ({
      "@type": "ListItem",
      position: index + 1,
      url: item.url,
      name: item.title,
  ...
  return (
    <>
      ...
      <main className="home-surface mx-auto w-full max-w-7xl px-4 py-6 md:px-6">
        <section className="v2-card p-4 md:p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-text-secondary">30-day trend plus per-source breakdown and full feed.</p>
              <div className="mt-2 flex flex-wrap gap-2 text-xs text-text-tertiary">
                <span className="rounded-card border border-border-primary bg-bg-secondary px-2 py-1">
                  total {formatNumber(mentions.length)}
                </span>
                <span className="rounded-card border border-border-primary bg-bg-secondary px-2 py-1">
                  24h delta +{formatNumber(mentions24h)}
                </span>
              </div>
            </div>
        ...
        <section className="v2-card mt-4 p-4 md:p-5">
          <div className="v2-mono text-[10px] text-text-tertiary">{"// PER-SOURCE BREAKDOWN - SPARKLINES"}</div>
          {countsBySource.length === 0 ? (
            ...
            <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {countsBySource.map(([source, count]) => {
                const perSourceTimeline = buildPerSourceTimeline(mentions, source as MentionSource);
                return (
                  <article key={source} className="rounded-card border border-border-primary bg-bg-secondary p-3">
                    ...
                    <div className="mt-2 flex h-8 items-end gap-0.5 overflow-hidden">
                      {perSourceTimeline.map((point) => (
                        <span
                          key={`${source}-${point.day}`}
                          className="block flex-1 rounded-sm bg-brand/70"
                          style={{ height: `${Math.max(2, point.count * 4)}px` }}
                        />
                      ))}
                    </div>
                  </article>
