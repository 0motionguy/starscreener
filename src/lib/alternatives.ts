...
export function buildAlternativesIntro(target: Repo, alts: Repo[], categoryName: string | null): string {
  ...
  const lead = `Looking for alternatives to ${target.fullName}? These are the top open-source ${cat} projects on TrendingRepo, ranked by cross-source momentum — GitHub star velocity weighted with mentions on Hacker News, Reddit, X, Bluesky and Dev.to.`;
  return `${lead}${ctx}`;
}
  ...
  out.push({
    ...
    a:
      list.length > 0
        ...
        : `TrendingRepo ranks same-category alternatives by a cross-source momentum score across GitHub, Hacker News, Reddit, X, Bluesky and Dev.to.`,
  });
