...
export function buildBestIntro(topic: BestTopic, repos: Repo[]): string {
  ...
  const method = ` Every project is open source and ranked by TrendingRepo's cross-source momentum score — GitHub star velocity weighted with live mentions on Hacker News, Reddit, X, Bluesky and Dev.to — not by raw star count, so newer breakouts surface alongside the established leaders.`;
...
export function buildBestFaq(topic: BestTopic, repos: Repo[]): FaqEntry[] {
  ...
  out.push({
    ...
    a:
      list.length > 0
        ? `As of ${todayLabel()}, the top-ranked are ${list} — ordered by TrendingRepo's cross-source momentum score across GitHub, Hacker News, Reddit, X, Bluesky and Dev.to.`
        : `TrendingRepo ranks these by a cross-source momentum score across GitHub, Hacker News, Reddit, X, Bluesky and Dev.to.`,
  });
  out.push({
