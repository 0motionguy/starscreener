...
// Pre-computed map of HF author slug -> real avatar URL (cdn-avatars.huggingface.co).
// Generated by scripts/enrich-hf-avatars.mjs at build/commit time so the
// page render is fully synchronous. Static import — bundled into the
// server build, no runtime fs access required.
import hfAvatarsData from "../../data/_meta/huggingface-avatars.json";
...
// Author -> real HF avatar URL, indexed by the raw HF slug (case-sensitive,
// matches the `author` field in the data files). Built once at module
// init — the JSON map can have ~2k entries but it's a flat object lookup
// so cost is negligible.
const HF_AUTHOR_AVATARS: Record<string, string | null> =
  (hfAvatarsData as { authors?: Record<string, string | null> }).authors ?? {};

/**
 * Project-specific logo for a HuggingFace org/user. Lookup order:
 *
 *   1. Pre-computed real avatar from data/_meta/huggingface-avatars.json
 *      (cdn-avatars.huggingface.co/...) — populated by
 *      scripts/enrich-hf-avatars.mjs against the public users +
 *      organizations APIs. Covers most authors on a fresh run.
 *   2. Mapped corporate domain favicon (NVIDIA, Google, Meta, DeepSeek...)
 *      via HF_AUTHOR_DOMAIN_MAP — historical fallback for the head of the
 *      distribution that pre-dates the avatar enrichment.
 *   3. HF platform mark — stable, on-brand last resort so unknown authors
 *      still get a recognisable "this is HuggingFace" tile rather than a
 *      Google-favicon globe or a blank slot.
 *
 * Returns null only when `author` is empty/whitespace — EntityLogo then
 * renders the monogram tile.
 */
export function huggingFaceAuthorLogoUrl(
  ...
  // 1. Real avatar from the enrichment pipeline (keyed on the raw HF slug
  // exactly as it appears in the data files — case sensitive).
  const enriched = HF_AUTHOR_AVATARS[raw];
  if (enriched) return enriched;

  // 2. Hand-curated corporate domain favicon for top-of-distribution orgs.
  ...
  // 3. Last-resort platform mark. Stable URL, on-brand, no broken icon.
