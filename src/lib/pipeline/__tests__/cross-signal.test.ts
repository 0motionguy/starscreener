...
test("devtoComponent: real repo with mentions ≥1 lights at least 0.4", () => {
  const mentioned = getDevtoLeaderboard().find((row) => row.count7d >= 1);
  assert.ok(mentioned, "expected dev.to fixture to include at least one mentioned repo");

  const score = devtoComponent(mentioned.fullName);
  assert.ok(score >= 0.4, `expected dev.to signal for ${mentioned.fullName}, got ${score}`);
});
