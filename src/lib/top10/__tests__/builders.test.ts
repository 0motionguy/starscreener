...
// ---------------------------------------------------------------------------
// BuildExtras decoration: NEW ENTRY badge + sparkline overlay
// ---------------------------------------------------------------------------

test("BuildExtras.priorTopSlugs adds NEW badge to absent items", () => {
  const repos = [
    makeRepo({
      fullName: "old/repo",
      name: "repo",
  ...
  const bundle = buildRepoTop10(repos, "7d", "cross-signal", {
    priorTopSlugs: new Set(["old/repo"]),
  });
  const old = bundle.items.find((i) => i.slug === "old/repo");
  const fresh = bundle.items.find((i) => i.slug === "fresh/face");
  assert.ok(old, "old/repo present");
  assert.ok(fresh, "fresh/face present");
  assert.equal(old!.badges.includes("NEW"), false);
  assert.equal(fresh!.badges.includes("NEW"), true);
});

test("BuildExtras with no priorTopSlugs leaves badges untouched", () => {
  const repos = [
    makeRepo({
      fullName: "any/thing",
      name: "thing",
  ...
  const bundle = buildRepoTop10(repos, "7d", "cross-signal", undefined);
