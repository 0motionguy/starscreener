...
- [x] Delete dead local-only maintenance scripts `scripts/purge-twitter-synthetic-data.ts` and `scripts/reset-data.mjs` — DELETED 2026-06-01. `scripts/scrape-sec-form-d.mjs` and `scripts/reconcile-repo-stores.mjs` remain live: collect-funding workflow, funding matcher docs, and reconciliation tests reference them.
...
- 2026-05-05 AGN-1610 [PM triage] Review productivity for AGN-734 (blocker reclassification retry):
  - Reclassification trigger: board comment `67cc2f1c-d7d5-4a48-83ef-1f0526d59617` requested PM to re-read AGN-1610 and clarify blocker cause.
  ...
  - Verified `npm run freshness:check` at `2026-05-05T06:07:51.732Z`: localhost reachable (`health=ok`) and product stale/degraded (`blocking_non_green=19`, `red=4`, `dead=2`, `Sentry: MISSING`).
  - [ ] AGN-1610 reclassification continuity follow-through (owner: PM triage). Done when AGN-1610 contains one refreshed evidence comment that explicitly classifies the AGN-734 blocker cause and one terminal status PATCH (`done` or `blocked`).
    Dependencies: platform/network owner restores reachability to `PAPERCLIP_API_URL=http://192.168.192.1:3100`; PM reruns issue thread fetch + PATCH immediately after API access is restored.
